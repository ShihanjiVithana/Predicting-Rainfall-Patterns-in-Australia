...
# Load and preprocess the cleaned weather data
cleaned_weather_data <- read.csv("E:/MSC/MSC NEW 2/cleaned_weather_data.csv")
cleaned_weather_data$Date <- as.Date(cleaned_weather_data$Date, format = "%Y-%m-%d")

# Convert RainToday to binary 0/1 format if necessary
...
for (location in locations) {
  ...
  if (nrow(location_data) == 0) {
    cat("No data for location:", location, "\n")
  ...
  if (nrow(location_data) > 0) {
    tryCatch({
      ...
      print(rf_importance_plot) # Debugging step: Print the plot
      
      # Save the plot
      ggsave(file.path(location_dir, "RF_Feature_Importance.png"), plot = rf_importance_plot, width = 8, height = 6)
      
      cat("Finished processing for location:", location, "\n")
      
    }, error = function(e) {
      cat("Error during processing for location:", location, "->", e$message, "\n")
    })
  } else {
    cat("No data left after omitting NAs for location:", location, "\n")
  }
}
